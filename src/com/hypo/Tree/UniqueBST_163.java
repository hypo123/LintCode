...
/**
 *	不同的二叉查找树
 *	问题描述:给出 n，问由 1...n 为节点组成的不同的二叉查找树有多少种?
 *	
 *  分析:
		1 	1 	 2 		3 	3
		\ 	\ 	/ \ 	/ 	/
	...
	当数组为 1, 2, 3, ..., n 时，基于以下原则的构建的 BST 树具有唯一性： 以 i 为根节点的树，
	其左子树由 [1, i-1] 构成，其右子树由 [i+1, n] 构成。
...
public class UniqueBST_163
{
    ...
    {
    	//结点个数为0个或1个时,只有一种BST树
    	if(n < 2) return 1;
    	
    	int[] f = new int[n+1];
    	
    	f[0] = 1;//0个结点,只有一种BST，即空树，f[0]=1 
    	f[1] = 1;//1个结点,只有一种BST，单个结点,f[1]=1
